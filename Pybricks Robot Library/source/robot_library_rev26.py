...
def initiatizeRobot(hubNum):
    ...
    if hubNum != tempHubNum:
        print("ERROR, hub number (",hubNum,") and hub name (",hubName,"/",tempHubNum,") do NOT match!")
        wait(2000)
        hub.system.shutdown()
...
async def moveAttachment(attachmentStr,degrees,speed,resetAngle,stallDetect,waitForPrompt):
...
async def moveAttachmentFC(attachmentStr,degrees,speed,stopMode,waitMode,resetAngle,stallDetect,waitForPrompt):
...
async def driveRobot(dist,speed, accel,waitForPrompt) -> bool :
...
async def turnRobot(dir,turnAccel, turnRate,waitForPrompt):
...
# drive robot in arc
#
async def driveRobotInArc(radius,mode,angleOrDist,waitForPrompt):
...
async def driveRobotInArcAndLift(radius,mode,angleOrDist,attachmentStr,degrees,aSpeed,resetAngle,stallDetect,waitForPrompt):
    ...
    if mode == "arc": 
        ...
        await multitask(driveBase.arc(radius,angle=angleOrDist,then=Stop.HOLD,wait=True),
                        moveAttachment(attachmentStr,degrees,aSpeed,resetAngle,False,False))
...
async def driveRobotAndLift(dist,dSpeed,accel,attachmentStr,degrees,aSpeed,resetAngle,stallDetect,waitForPrompt):
...
async def turnRobotAndLift(dir,turnAccel, turnRate,attachmentStr,degrees,aSpeed,resetAngle,stallDetect,waitForPrompt):
...
async def promptToContinue(waitForPrompt):
    ...
    if (waitForPrompt == True and continuePromptSkipCount <= 0):
        ...
        if prompt == 's':
            count=-1
            while (count < 0) :
                skipCount = input("How many Count to Skip? (#/0): ")
